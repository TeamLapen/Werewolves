...
public class WerewolvesBiomeFeatures {
    ...
    public static void registerBiomeFeatures() {
        ...
        magic_tree = registerFeature("magic_tree", Feature.TREE.configured(new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.magic_log.get().defaultBlockState()), new SimpleBlockStateProvider(ModBlocks.magic_leaves.get().defaultBlockState()), new BlobFoliagePlacer(FeatureSpread.fixed(2), FeatureSpread.fixed(0), 3), new StraightTrunkPlacer(4, 2, 0), new TwoLayerFeature(1, 0, 1)).ignoreVines().build()));
        magic_tree_big = registerFeature("magic_tree", Feature.TREE.configured(new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.magic_log.get().defaultBlockState()), new SimpleBlockStateProvider(ModBlocks.magic_leaves.get().defaultBlockState()), new FancyFoliagePlacer(FeatureSpread.fixed(2), FeatureSpread.fixed(4), 4), new FancyTrunkPlacer(3, 11, 0), new TwoLayerFeature(0, 0, 1, OptionalInt.of(4))).ignoreVines().heightmap(Heightmap.Type.MOTION_BLOCKING).build()));

        werewolf_heaven_trees = registerFeature("werewolf_heaven_trees", Feature.RANDOM_SELECTOR.configured(new MultipleRandomFeatureConfig(ImmutableList.of(jacaranda_tree.weighted(0.4f), Features.FANCY_OAK.weighted(0.1f), magic_tree_big.weighted(0.1f)), magic_tree)).decorated(Features.Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(10, 0.1F, 1))));
        wolfsbane = registerFeature("wolfsbane", Feature.FLOWER.configured(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.wolfsbane.get().defaultBlockState()), SimpleBlockPlacer.INSTANCE).tries(32).build()).count(FeatureSpread.of(-1, 4)).decorated(Features.Placements.ADD_32).decorated(Features.Placements.HEIGHTMAP_SQUARE).count(1));

        silver_ore = registerFeature("silver_ore", Feature.ORE.configured(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, ModBlocks.silver_ore.get().defaultBlockState(), 5)).range(45).squared().count(2));
    }
