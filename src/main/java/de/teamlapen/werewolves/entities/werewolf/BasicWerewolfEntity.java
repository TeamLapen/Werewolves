                ...
                if (fph.getMaxMinions() > 0) {
                    ...
                    if (this.getLevel() > 0) {
                        ...
                        if (heldItem.getItem() == ModItems.werewolf_minion_charm.get()) {
                            ...
                            player.displayClientMessage(new TranslationTextComponent("text.werewolves.basic_werewolf.minion.require_equipment", UtilLib.translate(ModItems.werewolf_minion_charm.get().getDescriptionId())), false);
                        }
                    }
                }
            });
                ...
                MinionWorldData.getData(lord.level).map(w -> w.getOrCreateController(fph)).ifPresent(controller -> {
                    if (controller.hasFreeMinionSlot()) {
                        if (fph.getCurrentFaction() == this.getFaction()) {
                            ...
                            ModEntities.werewolf_minion.map(a -> a.create(this.level)).ifPresent(minion -> {
