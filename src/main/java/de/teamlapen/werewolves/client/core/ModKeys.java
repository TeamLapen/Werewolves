        ...
        keyOpt.ifPresent(key -> {
            ...
            if (key == LEAP) {
                ...
                if (Helper.isWerewolf(player)) {
                    ...
                    if (entity instanceof LivingEntity && !player.abilities.invulnerable && werewolfOpt.map(w -> w.getForm().isTransformed() && w.getLevel() > 0 && w.canBite() && w.canBiteEntity(((LivingEntity) entity))).orElse(false)) {
