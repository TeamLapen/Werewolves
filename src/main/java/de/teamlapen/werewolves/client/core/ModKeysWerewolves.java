...
public class ModKeysWerewolves {
    ...
    public void handleInputEvent(InputEvent event) {
        ...
        if (SUCK.isKeyDown() && suck_down && !((ClientProxy) WerewolvesMod.proxy).getClientEventHandler().isZoomActive()) {
            ...
            if (mouseOver != null && !player.isSpectator() && FactionPlayerHandler.get(player).isInFaction(VReference.WEREWOLF_FACTION) && WerewolfPlayer.get(player).getSpecialAttributes().werewolf > 1) {
